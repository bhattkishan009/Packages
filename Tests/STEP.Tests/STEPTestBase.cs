...
namespace STEP.Tests
{
    public abstract class STEPTestBase : AbpIntegratedTestBase<STEPTestModule>
    {
        ...
        protected void UsingDbContext(Action<STEPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<STEPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<STEPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<STEPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<STEPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<STEPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<STEPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<STEPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<STEPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<STEPDbContext, Task<T>> func)
        {
            T result;
